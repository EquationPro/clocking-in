...
public class RepairCardController {
    ...
    public ResultMap listByPage(Integer page, Integer limit) {
        PageBean<RepairCard> pageBean = service.listByPage(page, limit);
        return ResultMap.layuiData(pageBean.getCountTotal(), pageBean.getData());
    }

    @RequestMapping("/insert")
    @ResponseBody
    public ResultMap insert(String repEmp, String repTime, String repReason,HttpSession session) throws ParseException {
        //获取到员工编码
        String empCode = (String) session.getAttribute("empCode");
        //进行打卡
        Integer rs = service.insertRepairCard(new RepairCard(repEmp, repTime, repReason));
        //获取到员工姓名
        ...
        if (rs > 0) {
            ...
            if (checkReport.getCheBegin() == null) {
                ...
                checkReport.setCheBegin(repTime);
                //获取到小时数
                ...
                checkReport.setCheName(empName);
                checkReport.setCheEnd(repTime);
                //此处为最终打卡,获取首次打卡时间进行计算
                String cardFirst = checkReport.getCheBegin();
                //计算出勤小时数
                int gap = CountCardTimeUtil.countCardTimeUtil(cardFirst, repTime);
                if (gap >= 8.5) {
...
//                Date parse = all.parse(punTime);
    ...
    @ResponseBody
    public ResultMap deleteAll(String repIds) {
        String[] split = repIds.split(",");
        for (String repId : split) {
            service.deleteRepairCard(Integer.valueOf(repId));
        }
        return ResultMap.justOk();
    }

    @RequestMapping("/loadById")
    public String loadById(Integer repId, HttpSession session) {
        RepairCard repairCard = service.loadRepairCard(repId);
        session.setAttribute("repairCard", repairCard);
        return "/repaircard_update.jsp";
    }

    @RequestMapping("/update")
    @ResponseBody
    public ResultMap update(String repEmp, String repTime, String repReason, HttpSession session) {
        RepairCard repairCard = (RepairCard) session.getAttribute("repairCard");
        Integer repId = repairCard.getRepId();
        Integer rs = service.updateRepairCard(new RepairCard(repId,repEmp,repTime,repReason));
        if (rs > 0) {
            return ResultMap.justOk();
        } else {
            return ResultMap.justFail();
    ...
    @RequestMapping("/searchRepairCard")
    @ResponseBody
    public ResultMap search(String repTimeBegin, String repTimeEnd, String repEmp,int page, int limit) {
        PageBean<RepairCard> pageBean = service.listLimitTimeByPage(repTimeBegin, repTimeEnd, repEmp, page, limit);
        return ResultMap.layuiData(pageBean.getCountTotal(), pageBean.getData());
    }
}
