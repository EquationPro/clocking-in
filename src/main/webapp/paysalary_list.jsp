    ...
    layui.use('table', function () {
        ...
        table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'payId', title: 'ID', sort: true, fixed: 'left', totalRowText: '合计'}
                , {field: 'payEmp', title: '领薪人编码'}
                , {field: 'payEmpName', title: '领薪人姓名'}
                , {field: 'paySal', title: '薪水'}
                , {field: 'payBegin', title: '计算开始时间'}
                , {field: 'payEnd', title: '计算结束时间'}
        ...
        var $ = layui.$, active = {
            reload: function () {
                ...
                table.reload('testReload', {
                    url: '/paySalary/searchPaySalary',
                    page: {
                        curr: 1 //重新从第 1 页开始
                    }
                    , where: {
                        payEmpNameOrPayEmp: demoReload.val()  , //data.field.userName  //obj.data
                        payBegin: demoReloadBegin.val(),
                        payEnd: demoReloadEnd.val()
                    }
                }, 'data');
            }
        };
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                ...
                case 'isAll':
                    ...
                    break;
                case 'add':
                    xadmin.open("添加记录", "paysalary_insert.jsp", 600, 650);
                    break;
            }
        });
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                $.ajax({
                    type: 'get',
                    url: '/paySalary/delete',
                    dataType: 'json',
                    data: {
                        payId: data.payId  //obj.data
                    },
                    success: function (data) {
                        if (data.code == 200) {
                            obj.del();
                ...
                xadmin.open("修改信息", '/paySalary/loadById?payId=' + data.payId, 700, 400)
            }
        });
    });
</script>
